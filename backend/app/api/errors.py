...
_STATUS_BY_ERROR: dict[type[DomainError], int] = {
    ...
    # Correct credentials, account just is not usable yet — the same class of "authenticated,
    # not yet permitted" as a deactivated account, hence the same status.
    ...
    # A Google ID token that fails signature/audience/expiry verification is exactly the same
    # shape of failure as a bad password: 401, with the same WWW-Authenticate treatment below.
    GoogleTokenInvalidError: status.HTTP_401_UNAUTHORIZED,
    GoogleSignInNotConfiguredError: status.HTTP_503_SERVICE_UNAVAILABLE,
